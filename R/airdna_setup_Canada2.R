...
multilistings <- function(daily){
  ...
  daily <-
    ...
    mutate(MLnew = ifelse(n()>=2 & Listing_Type=="Entire home/apt", TRUE,
...
raffle_list <- function(points, polys, distance = distance, diagnostic = diagnostic, cores = cores) {
...
raffle <- function(points, polys, distance = 200, diagnostic = FALSE, cores = 1, max_chunk = 50000){
  ...
  if(nrow(points)>max_chunk){
  
    points_list <- split(points,ceiling(nrow(points)/max_chunk))
    time1 <- Sys.time()
    points_list <- lapply(points_list,
                          raffle_list,
                          polys = polys,
                          distance = distance,
                          diagnostic = diagnostic,
                          cores = cores)
    points <- do.call(rbind, points_list, polys)
    points
  } else {
    points <- raffle_list(points,
                          polys = polys,
                          distance = distance,
                          diagnostic = diagnostic,
                          cores = cores)
...
time3 <- Sys.time()
Canada_property <- raffle(Canada_points, Canada_polys, cores = 6)
time4 <- Sys.time()
