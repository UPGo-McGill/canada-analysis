...
eh_listings <- function(daily_prop) {
  eh_listings_tidy_PID <- summarise(group_by(daily_prop[daily_prop$Listing_Ty=="Entire home/apt",], monthYear,City, Property_I), n()) #summarize daily_prop by monthYear, city, and Property_ID to get count of EH Rs per PID
  eh_listings_tidy <- aggregate(r_eh_tidy_PID$`n()`, list(r_eh_tidy_PID$monthYear, r_eh_tidy_PID$City), length) #tidy form of EH R counts for R>0 per month per city
